...
import (
	...
	"github.com/Jiraffe7/proglog/internal/agent"
	"github.com/Jiraffe7/proglog/internal/config"
	"github.com/Jiraffe7/proglog/internal/loadbalance"
...
func TestAgent(t *testing.T) {
	...
	// wait until replication has finished before consuming from leader
	time.Sleep(3 * time.Second)
	...
	// example to show that current replication implementation is naive and replicates in a cycle
	// consumeResponse, err = leaderClient.Consume(context.Background(), &api.ConsumeRequest{Offset: produceResponse.Offset + 1})
	// assert.Nil(t, consumeResponse)
	// assert.Error(t, err)
	// got := grpc.Code(err)
