...
func TestMultipleNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		require.NoError(t, err)
		if i != 0 {
			// test manually calls join on leader, not from membership service discovery.
			// Question arose when only leader should add followers,
			// but the event will be sent to all followers?
